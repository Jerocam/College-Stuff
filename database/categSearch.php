<?php
    ...
    $results = $conn->query("SELECT COUNT(id) FROM new_product WHERE product_name LIKE '%{$product_search}%'");//get total number of records from database
    $get_total_rows = $results->fetch_row(); //hold total records in variable
    $total_pages = ceil($get_total_rows[0]/$item_per_page);//break records into pages
    $page_position = (($page_number-1) * $item_per_page);
    
    echo search_products($conn, $product_search, $page_position, $item_per_page);
    
    function search_products($conn, $product_search, $page_position, $item_per_page){  
        $output = '';  
        $sql = "SELECT * FROM new_product WHERE product_name LIKE '%{$product_search}%' LIMIT $page_position, $item_per_page";  
        ...
        while($row = mysqli_fetch_array($result)){
            ...
            $output .= '<div class="nlist col-md-3 '.$load_categ.'">
                <div class="card mb-3">
                    <div class="dRow">
                        <div class="cl1"> 
    ...
    function paginate_function($item_per_page, $current_page, $total_records, $total_pages){
        ...
        if($total_pages > 0 && $total_pages != 1 && $current_page <= $total_pages){ //verify total pages and current page number
            $pagination .= '<ul class="pagSearch pagination justify-content-center">';
