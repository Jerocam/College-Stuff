...
$results = $conn->query("SELECT COUNT(id) FROM new_product WHERE product_categ='$categ'");
$get_total_rows = $results->fetch_row();
$total_pages = ceil($get_total_rows[0]/$item_per_page);
$page_position = (($page_number-1) * $item_per_page);

echo categ_products($conn, $categ, $page_position, $item_per_page);

function categ_products($conn, $categ, $page_position, $item_per_page){  
    $output = '';  
    $sql = "SELECT * FROM new_product WHERE product_categ='$categ' LIMIT $page_position, $item_per_page";  
...
function paginate_function($item_per_page, $current_page, $total_records, $total_pages){
    ...
    if($total_pages > 0 && $total_pages != 1 && $current_page <= $total_pages){ //verify total pages and current page number
        $pagination .= '<ul class="pagElectronics pagination justify-content-center">';
